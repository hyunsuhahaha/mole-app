...
const averages = [
  { days: 5, color: "#E2A400" },
  { days: 20, color: "#3478D4" },
  { days: 60, color: "#3D9B67" },
  { days: 120, color: "#8B8B8B" },
];
...
export function PriceCandlestickChart({
  ...
  const highs = points.map(
    (point) => point.high ?? Math.max(point.open ?? point.close, point.close),
  );
  const lows = points.map(
    (point) => point.low ?? Math.min(point.open ?? point.close, point.close),
  );
  ...
  const y = (value: number) =>
    pad.top + ((max - value) / (max - min)) * (priceBottom - pad.top);
  ...
  if (points.length < 2)
    return (
      <Text style={s.noData}>캔들차트를 그리기에 가격 자료가 부족해요.</Text>
    );
  ...
  return (
    <View>
      <View style={s.chartReadout}>
        <View>
          <Text style={s.readoutDate}>{active.date}</Text>
          <Text style={s.readoutValue}>
            {currency === "USD" ? "$" : ""}
            {active.close.toFixed(2)}
          </Text>
        </View>
        <Text style={[s.readoutChange, change < 0 && s.down]}>
          {change >= 0 ? "+" : ""}
          {change.toFixed(2)}%
        </Text>
      </View>
      <Text style={s.ohlc}>
        시 {open.toFixed(2)} 고 {high.toFixed(2)} 저 {low.toFixed(2)} 종{" "}
        {active.close.toFixed(2)}
      </Text>
      <View style={s.legend}>
        {averages.map((average) => (
          <View key={average.days} style={s.legendItem}>
            <View style={[s.legendLine, { backgroundColor: average.color }]} />
            <Text style={s.legendText}>MA{average.days}</Text>
          </View>
        ))}
      </View>
      <View
        onLayout={(event) => setWidth(event.nativeEvent.layout.width)}
        style={s.priceSvgWrap}
      ...
      >
        <Svg width={width} height={chartHeight}>
          ...
          {points.map((point, index) => {
            ...
            const bodyHeight = Math.max(
              1,
              Math.abs(y(pointOpen) - y(point.close)),
            );
            const volumeHeight =
              ((point.volume ?? 0) / maxVolume) * (volumeBottom - volumeTop);
            return (
              <Fragment key={point.date}>
                <Line
                  ...
                  y2={y(pointLow)}
                  stroke={color}
                  strokeWidth={Math.max(1, candleWidth * 0.18)}
                />
                <Rect
          ...
          {maLines.map((average) => {
            const path = average.values
              .map((value, index) =>
                value == null
                  ? ""
                  : `${average.values.slice(0, index).some((item) => item != null) ? "L" : "M"}${x(index).toFixed(1)},${y(value).toFixed(1)}`,
              )
              .filter(Boolean)
              .join(" ");
            return path ? (
          ...
          <Line
            ...
            stroke={colors.line}
          />
          <SvgText x={pad.left} y="222" fontSize="8" fill={colors.muted}>
            거래량
          </SvgText>
          <SvgText x={pad.left} y="299" fontSize="9" fill={colors.muted}>
        ...
        {Platform.OS === "web" && (
          <Pressable
            accessibilityLabel="가격 차트에서 날짜 선택"
            onPress={(event) =>
              setFromX(
                event.nativeEvent.locationX ??
                  (event.nativeEvent as unknown as { offsetX?: number })
                    .offsetX ??
                  Number.NaN,
              )
            }
            style={s.webPriceTarget}
          />
        )}
      </View>
      <View style={s.rangeRow}>
        {ranges.map((range) => (
          <Pressable
            ...
            style={[s.range, days === range.days && s.rangeActive]}
          >
            <Text
              style={[s.rangeText, days === range.days && s.rangeTextActive]}
            >
              {range.label}
            </Text>
          </Pressable>
...
export function RevenueBarChart({ data }: { data: RevenuePoint[] }) {
  ...
  return (
    <View>
      <View style={s.chartReadout}>
        <View>
          <Text style={s.readoutDate}>{active.period}</Text>
          <Text style={s.readoutValue}>{active.display}</Text>
        </View>
        <Text style={s.revenueTag}>분기 매출</Text>
      </View>
      <View
        onLayout={(event) => setWidth(event.nativeEvent.layout.width)}
        style={s.revenueSvgWrap}
      >
            ...
            return (
              <Fragment key={point.period}>
                <Rect
                  ...
                  height={height}
                  rx="5"
                  fill={activeBar ? colors.green : colors.soilLight}
                  onPress={
                    Platform.OS === "web" ? undefined : () => setSelected(index)
                  }
                />
                <SvgText
                  x={barX + barWidth / 2}
                  ...
                  textAnchor="middle"
                  fontSize="9"
                  fontWeight={activeBar ? "700" : "400"}
                  fill={activeBar ? colors.ink : colors.muted}
                >
          ...
          >
            {points.map((point, index) => (
              <Pressable
                ...
                accessibilityLabel={`${point.period} 매출 보기`}
                onPress={() => setSelected(index)}
                style={s.webRevenueTarget}
              />
            ))}
          </View>
        )}
      </View>
      <Text style={s.dragHelp}>
        막대를 누르면 해당 분기의 정확한 매출을 볼 수 있어요.
      </Text>
    </View>
  );
}
