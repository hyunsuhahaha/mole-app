  ...
  return (
    <SafeAreaView style={s.page}>
      <View style={s.header}>
        ...
        <Text style={s.count}>
          {stocks.data?.count
            ? `${stocks.data.count.toLocaleString()}개`
            : market === "US"
              ? "미국 주식"
              : "국내 주식"}
        </Text>
      ...
      <View style={s.marketTabs}>
        {(
          ...
          <Pressable
            ...
            accessibilityState={{ selected: market === value }}
            onPress={() => setMarket(value)}
            style={[s.marketTab, market === value && s.marketTabActive]}
          >
            <Text
              style={[
                s.marketTabText,
                market === value && s.marketTabTextActive,
              ]}
            >
              {label}
            </Text>
      ...
      >
        ...
        <Text style={s.sectionHelp}>
          {debounced
            ? "종목코드와 회사 이름을 함께 찾았어요."
            : `${market === "US" ? "미국" : "국내"} 종목을 코드 순서로 보여드려요. 검색하면 회사명과 종목코드로 바로 찾을 수 있어요.`}
        </Text>
        {stocks.isLoading &&
        ...
        {stocks.isError && (
          <View style={s.stateBox}>
            ...
            <Text style={s.stateText}>
              PC의 데이터 서버가 켜져 있는지 확인해주세요.
            </Text>
            <Pressable
              accessibilityRole="button"
              onPress={() => stocks.refetch()}
            >
              <Text style={s.retry}>다시 불러오기 →</Text>
            </Pressable>
          </View>
        )}
        {!stocks.isLoading && !stocks.isError && !stocks.data?.items.length && (
          <View style={s.stateBox}>
            <Text style={s.stateTitle}>일치하는 회사를 찾지 못했어요</Text>
            <Text style={s.stateText}>
              영문 회사명이나 종목코드로 다시 검색해보세요.
            </Text>
          </View>
        )}
        ...
        <Text style={s.disclaimer}>
          {market === "US"
            ? "미국 회사 공시를 검색한 결과이며 투자 추천 순위가 아니에요."
            : `${stocks.data?.source ?? "국내 상장 종목 목록"}이며 투자 추천 순위가 아니에요.`}
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
...
function StockRow({ stock }: { stock: StockSearchItem }) {
  ...
  const summary =
    stock.operating_income == null
      ? "본업 수익 자료 확인 필요"
      : stock.operating_income > 0
        ? "본업에서 돈을 벌고 있어요"
        : "본업은 아직 적자예요";
  return (
    <Pressable
    ...
    >
      <View style={s.symbol}>
        <Text style={s.symbolText}>{stock.ticker.slice(0, 2)}</Text>
      </View>
      <View style={s.rowBody}>
        ...
        <Text style={s.summary}>
          {stock.market === "KR"
            ? `${stock.exchange ?? "KRX"} 상장 · 가격 조회`
            : summary}
        </Text>
      </View>
      {stock.market !== "KR" && (
        <View style={s.metric}>
          <Text style={[s.growth, !positive && s.down]}>
            {stock.revenue_growth == null
              ? "—"
              : `${positive ? "+" : ""}${stock.revenue_growth.toFixed(1)}%`}
          </Text>
